...
class ProductionOrderCannotBeCancelledException(Exception):
    def __init__(self, order_id: int, status: str):
        super().__init__(
            f"Production order with id={order_id} cannot be cancelled "
...
class ProductionOrderCannotBeExecutedException(Exception):
    def __init__(self, order_id: int, status: str):
        super().__init__(
            f"Production order with id={order_id} cannot be executed "
            f"from status '{status}'"
        )
        self.order_id = order_id
        self.status = status
