...
class Item:
   ...
   # ── Attribute Mutators ────────────────────────────────────────
    ...
    def update(self, **kwargs):
        """
        Operación de update parcial del item
        """
        allowed_fields = {
            "name",
            "item_type_id",
    ...
    def can_be_sold(self) -> bool:
        """ 
        Devuelve True si el ítem está activo y marcado como vendible.
        """
        return self.is_sellable and self.is_active()

    def can_be_purchased(self) -> bool:
        """
        Devuelve True si el ítem está activo y marcado como comprable.
        """
        return self.is_purchasable and self.is_active()

    def can_be_manufactured(self) -> bool:
        """
        Devuelve True si el ítem está activo y marcado como fabricable.
        """
    ...
    def is_active(self) -> bool:
        """
        Verifica si el ítem está en estado ACTIVE.
        """
        return self.status == ItemStatus.ACTIVE
    
    def is_deleted(self) -> bool:
        """
        Verifica si el ítem está en estado DELETED.
        """
    ...
    def _guard_not_deleted(self, operation: str) -> None:
        """
        Lanza error si se intenta operar sobre un ítem ya eliminado.
        """
        if self.is_deleted():
            raise ValueError(
